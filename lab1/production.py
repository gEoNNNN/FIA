...
def backward_chain(rules, hypothesis, verbose=False):
    ...
    def backward_chain_recursive(goal, rules, visited=None):
        if visited is None:
            visited = set()
        ...
        # Find all rules that could prove this goal
        possible_rules = []
        ...
        for rule in rules:
            if hasattr(rule, 'consequent') and rule.consequent():
                for consequent in rule.consequent():
                    ...
                    bindings = match(consequent, goal)
                    if bindings is not None:
                        possible_rules.append((rule, bindings))
                        
        if not possible_rules:
            # Base case: goal cannot be proven by any rule
            return goal
            
        # Build OR tree of all possible ways to prove the goal
        branches = []
        
        for rule, bindings in possible_rules:
            antecedent = rule.antecedent()
            
            # Apply bindings to antecedent
            try:
                instantiated_antecedent = instantiate(antecedent, bindings)
            except:
                instantiated_antecedent = antecedent
                
            # Recursively backward chain on the antecedent
            if isinstance(instantiated_antecedent, AND):
                sub_goals = []
                for condition in instantiated_antecedent:
                    sub_goal = backward_chain_recursive(condition, rules, visited.copy())
                    sub_goals.append(sub_goal)
                if sub_goals:
                    branches.append(AND(*sub_goals))
            elif isinstance(instantiated_antecedent, OR):
                sub_goals = []
                for condition in instantiated_antecedent:
                    sub_goal = backward_chain_recursive(condition, rules, visited.copy())
                    sub_goals.append(sub_goal)
                if sub_goals:
                    branches.append(OR(*sub_goals))
            else:
                # Single condition
                sub_goal = backward_chain_recursive(instantiated_antecedent, rules, visited.copy())
                branches.append(sub_goal)
    ...
    return backward_chain_recursive(hypothesis, rules)
